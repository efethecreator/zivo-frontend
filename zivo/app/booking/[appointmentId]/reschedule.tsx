...
const generateCalendarDays = (businessShifts = [], currentDate = new Date()) => {
  const days = []
...
const RescheduleScreen = () => {
  ...
  const calendarDays = useMemo(() => generateCalendarDays(businessShifts, currentMonth), [businessShifts, currentMonth])
  ...
  const filteredTimeSlots = useMemo(() => {
    ...
    let dayOfWeek = date.getDay()
    if (dayOfWeek === 0) dayOfWeek = 7
    const shift = businessShifts.find((s) => s.dayOfWeek === dayOfWeek && s.isActive)
    ...
    return timeSlots.filter((t) => (start < end ? t >= start && t < end : t >= start || t < end))
  }, [selectedDate, businessShifts, timeSlots])

  const rescheduleMutation = useMutation({
    mutationFn: ({ id, appointmentTime }) => rescheduleAppointment(id, appointmentTime),
