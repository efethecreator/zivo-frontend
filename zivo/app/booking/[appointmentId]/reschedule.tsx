...
import { getAppointmentById, rescheduleAppointment } from "../../../services/appointment.service"
import { getBusinessShifts, getShiftTimes } from "../../../services/businessShift.service"
...
const generateCalendarDays = (businessShifts: any[] = [], currentDate = new Date()) => {
  const days = []
  ...
  for (let i = 1; i <= lastDayOfMonth.getDate(); i++) {
    ...
    const isBusinessOpen = businessShifts.some((shift) => shift.dayOfWeek === dayOfWeek && shift.isActive)
...
const RescheduleScreen = () => {
  ...
  const { data: appointment, isLoading: isAppointmentLoading } = useQuery({
    ...
    enabled: !!appointmentId && typeof appointmentId === "string",
  })
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      if (appointment?.services && appointment.services.length > 0) {
        try {
          const servicePromises = appointment.services.map((service) => getServiceById(service.serviceId))
  ...
  const { data: businessShifts } = useQuery({
    ...
    queryFn: () => getBusinessShifts(appointment?.businessId as string),
  ...
  const filteredTimeSlots = useMemo(() => {
    ...
    let dayOfWeek = date.getDay()
    if (dayOfWeek === 0) dayOfWeek = 7
    const shift = businessShifts.find((s: any) => s.dayOfWeek === dayOfWeek && s.isActive)
    ...
    return timeSlots.filter((t: string) => (start < end ? t >= start && t < end : t >= start || t < end))
  }, [selectedDate, businessShifts, timeSlots])

  const rescheduleMutation = useMutation({
    mutationFn: ({ id, appointmentTime }: { id: string; appointmentTime: string }) =>
  ...
  const formattedPrice =
    typeof appointment.totalPrice === "number" ? `$${appointment.totalPrice.toFixed(2)}` : "Price not available"

  return (
    <SafeAreaView style={styles.container}>
      <AnimatedHeader title="Reschedule Appointment" onBack={() => router.back()} scrollY={scrollY} />
      ...
      >
            ...
            {calendarDays.map((day, idx) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  selectedDate?.day === day.day && selectedDate?.month === day.month && styles.selectedDateCell,
              ...
              >
                <Text
                  style={[
                    ...
                    selectedDate?.day === day.day && selectedDate?.month === day.month && styles.selectedDateText,
        ...
        <TouchableOpacity
          style={[styles.rescheduleButton, (!selectedDate || !selectedTime) && styles.disabledButton]}
