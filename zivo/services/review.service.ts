...
export const calculateBusinessRating = (reviews: Review[], businessId: string) => {
  ...
  const businessReviews = reviews.filter(
    (review) => !review.isDeleted && review.appointment && review.appointment.businessId === businessId,
  )
...
export const hasUserReviewedAppointment = (reviews: Review[], appointmentId: string) => {
  return reviews.some((review) => review.appointmentId === appointmentId && !review.isDeleted)
}
